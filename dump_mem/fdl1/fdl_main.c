...
//let's dump mem
	for(i=0;i<128;i++)
	{
		...
		packet = FDL_MallocPacket();
		packet->packet_body.type = 0x11;
		packet->packet_body.size = 512;
		FDL_memcpy(packet->packet_body.content, (char *)(void *)(0x100000 + i*512), 512);//bootrom 100000-10ffff,64kb
		FDL_SendPacket(packet);
		FDL_FreePacket(packet);
	}
    for (;;)
